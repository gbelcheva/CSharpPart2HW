namespace _04.HashtableImplementation
{
    ...
    public class HashTable<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>
    {
        ...
        private const float ResizingThreshold = 0.75f;
        ...
        private void Expand()
        {
            var maxLength = (int)(this.table.Length * ResizingThreshold);
            ...
            if (this.Count >= maxLength)
            {
                ...
                foreach (var oldItem in oldTable)
                {
                    if (oldItem != null)
                    {
                        foreach (var keyValuePair in oldItem)
                        {
