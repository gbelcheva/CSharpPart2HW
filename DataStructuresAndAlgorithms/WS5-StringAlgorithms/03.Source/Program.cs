namespace _03.Source
{
    ...
    public class Hash
    {
        ...
        public void Add(char c, int index)
        {
            ...
            if ('A' <= c && c <= 'Z')
            {
                //this.Hash1 += (this.Hash1 * Base1) % Mod1;
        ...
        public void Remove(char c, int index)
        {
            ...
            else
            {
                ...
                this.Hash2 += Mod2 * Mod2 - (ulong)this.distances[index] * this.power2[this.power2.Length - 1];
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Hash hashPattern = new Hash(pattern);
            Hash hashWindow = new Hash(text.Substring(0, pattern.Length));


            List<int> matches = new List<int>();
            if (hashPattern.Hash1 == hashWindow.Hash1 &&
                hashPattern.Hash2 == hashWindow.Hash2)
            {
                matches.Add(1);
            }
            ...
            {
                hashWindow.Add(text[i], i);
                hashWindow.Remove(text[i - pattern.Length], i - pattern.Length);
